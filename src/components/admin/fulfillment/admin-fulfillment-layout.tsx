...
import { ManualProspectUpload } from "./manual-prospect-upload"
import { Building2, X } from "lucide-react"

export function AdminFulfillmentLayout() {
    ...
    return (
        <div className="flex flex-col h-full max-w-5xl mx-auto w-full">
            <header className="mb-8">
                <h1 className="text-2xl font-bold text-white mb-2">Outreach Research Fulfillment</h1>
                <p className="text-gray-400">Manually upload researched leads for client companies.</p>
            </header>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 h-full">
                ...
                <div className="lg:col-span-2 space-y-6">
                    <div className="bg-[#111] border border-white/5 rounded-xl p-4 h-full">
                        <h2 className="text-sm font-bold text-white uppercase tracking-wider mb-4 border-b border-white/5 pb-2">
                            2. Upload Data
                        </h2>
                        ...
                        {selectedCompany ? (
                            <div className="animate-in fade-in slide-in-from-left-4 duration-300">
                                <ManualProspectUpload empresaId={selectedCompany.id} />
                            </div>
                        ) : (
                            <div className="flex flex-col items-center justify-center h-64 text-center border-2 border-dashed border-white/5 rounded-lg bg-white/[0.02]">
                                ...
                                <p className="text-sm text-gray-500 max-w-xs">
                                    Please select a client company from the left panel to begin uploading prospects.
                                </p>
                            </div>
                        )}
                    </div>
